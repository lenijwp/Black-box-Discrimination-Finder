...
for Dat in Dat_list:
    for Exeiter in range(5):
        ...
        def Clip(x):
            ...
            return x
        def Choose_argmax(x):
            res0 = Querytargetmodel(x)[2] #get the gradient
            y = x.copy()
            ans = 0.0
        ...
        while alter_visit>0:
            print("go!!!")
           # data=Get_ini_seed(2000)
            Global_Generation(data,4,200,10,1)
            
        end_time=time.time()

        outfile_path2='Demo/Results/Numbers/ADF/'+Dat.__class__.__name__+'_'+str(Exeiter)+'.txt'
    ...
    # f = open('Demo/Results/ADF_result.txt','w+')
       ...
       # f2 = open(outfile_path2,'w+')
        '''
        st=''
        for i in range(len(result_for_vis)):
            st=st+' '+str(result_for_vis[i])
       ...
       # f2.close
        ...
        outfile_path='Demo/Results/ADF/'+Dat.__class__.__name__+'_'+str(Exeiter)+'.txt'
        print(outfile_path)
    # f = open('Demo/Results/ADF_result.txt','w+')
