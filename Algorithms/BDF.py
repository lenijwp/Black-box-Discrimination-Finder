...
for Dat in Dat_list:
    for Exeiter in range(5):
        ...
        sub_model_path = 'Demo/Shadowmodels/Initial_shadow_model.h5'  
        ...
        sub_model = K.models.load_model(sub_model_path)  # !only for initialization,before using it we will reflash it by shadow models.
        ...
        def Define_model():
            ...
            model.add(Dense(8, activation='relu'))  #new
            ...
            model.add(Dense(16, activation='relu'))  #new
#            model.add(Dense(4, activation='relu'))
                ...
                for iters in range(Max_iters):
                    ...
                    if Access_filter(x, sub_model, optflag) == True:
                        ...
                        if flag[0] == True:
                            ...
                            Local_Generation(Target_model_path, Sub_model_path, [x].copy(), 200, 160, 1, optflag)
        ...
        def GET_WEIGHT(density):
            res=np.zeros(len(density))
            for i in range(len(density)):
                #res[i]=density[i]/(density[i]+len(density)*0.1)
                res[i]=(density[i]+0.1)/(np.sum(density)+len(density)*0.1)
        ...
        inidata = Get_ini_seeds(num_ini_seeds)
        Substitute_DNN_Train(2, inidata, 1, target_model_path, sub_model_path, 0.2, 200)
        ...
        while True:
            ...
            Seeds_number=np.zeros(Scnum)
            Discri_number=np.zeros(Scnum)
            Density=np.zeros(Scnum)
            for i in range(len(Seeds)):
                Datas[Labs[i]].append(Seeds[i])
                Seeds_number[Labs[i]]+=1.0
                if Seeds[i] in all_dis:
                    Discri_number[Labs[i]]+=1.0

            for i in range(Scnum):
                Density[i]=Discri_number[i]/Seeds_number[i]
                outimfo="Cluster "+str(i)+"  "+str(Seeds_number[i])+"   "+str(Discri_number[i])+"   "+str(Density[i])
                print(outimfo)
                pri_cluster=pri_cluster+outimfo+'\n'

            WEIGHT=GET_WEIGHT(Density)
            ...
            for i in range(4 + step_local):
                ...
                for j in range(Scnum):
                    ...
                    if alter_visit <= 0:
                        break
                    if len(all_dis) - base_dis > n_glab[j] * 30:
                        n_glab[j] += 5
                        continue
        ...
        outfile_path2='Demo/Results/Numbers/BDF/'+Dat.__class__.__name__+'_'+'Jacobi_2_'+str(num_ini_seeds)+'_'+str(Exeiter)+'.txt'
        with open(outfile_path2,'w+') as f2:
           # print(str(result_for_vis))
            ...
            f2.write('\n')
            f2.write(pri_cluster)
        print(outfile_path2)
    # f = open('Demo/Results/ADF_result.txt','w+')
        print(str(result_for_vis))

        outfile_path='Demo/Results/BDF/'+Dat.__class__.__name__+'_'+'Jacobi_2_'+str(num_ini_seeds)+'_'+str(Exeiter)+'.txt'
        print(outfile_path)
    # f = open('Demo/Results/ADF_result.txt','w+')
